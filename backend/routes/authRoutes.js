...
router.put('/addToStation', async (req, res) => {
  const { cnic, stationName } = req.body;
  try {
    const station = await Station.findOneAndUpdate({ name: stationName }, { assignedPersonnel: Number(cnic) }, { new: true });
    if (!station) {
      return res.status(404).json({ message: 'Station not found' });
    }
    res.status(200).json({ message: "Successfully Added to station" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Server Error' });
  }
